...
if __name__ == '__main__':
    ...
    if args.controller_type == 'sgd':
        ...
        optim = PIDOptimizer(ddpm.parameters(), lr=args.learning_rate, weight_decay=args.weight_decay, momentum=0.9, I_pid=I_pid, D_pid=D_pid)
    elif args.controller_type == 'lpfsgd':
        optim = LPFSGDOptimizer(ddpm.parameters(), lr=args.learning_rate, weight_decay=args.weight_decay, lpf_sgd=True)
    elif args.controller_type == 'hpfsgd':
        optim = HPFSGDOptimizer(ddpm.parameters(), lr=args.learning_rate, weight_decay=args.weight_decay, hpf_sgd=True)
    elif args.controller_type == 'fuzzypid':
        optim = FUZZYPIDOptimizer(ddpm.parameters(), lr=args.learning_rate, weight_decay=args.weight_decay, momentum=0.9, I_pid=I_pid, D_pid=D_pid)
    ...
    for ep in range(n_epoch):
        ...
        torch.save(ddpm.state_dict(), model_path + f"model_{ep}.pth")
        print('saved model at ' + model_path + f"model_{ep}.pth")
